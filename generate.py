...
for j in range(20):
    ...
    images = [torch.ones(1, 3, 128, 128).cuda() * -1]

    source_image = generator(source_code, step=5, alpha=1, mean_style=mean_style, style_weight=0.7)
    target_image = generator(target_code, step=5, alpha=1, mean_style=mean_style, style_weight=0.7)
